...
```{r Parameter - Preferences - Initial year, echo = FALSE}
...
param_base = param_base %>% 
  group_by(Country) %>% 
  mutate(omega = ifelse(Year == 1970 & Country == "France", omega.init, omega.raw %>%
                          subset(Year > 1970) %>% 
                          mean(na.rm = TRUE)),
         beta = beta.init) %>% 
...
```
```{r Graph - BModel Var 70-10 - Public, echo = FALSE}
final_dev_pub = final %>% 
  group_by(Country) %>%
  mutate(# taxrev = tau*Y
         # unempsp = b*Ny*u/tau/Y,
         # healthsp = h*No/tau/Y,
    outside = b/(1-tau),
         eta_dev = eta/eta[Year == 1970] -1,
         # taxrev_dev = taxrev/taxrev[Year == 1970] -1,
         # unempsp_dev = unempsp/unempsp[Year == 1970] -1,
         # healthsp_dev = healthsp/healthsp[Year == 1970] -1
         outside_dev = outside/outside[Year == 1970] -1,
         w_dev = w/w[Year == 1970] -1,
         # h_dev = h/h[Year == 1970] -1,
         ) %>% 
  select(Country, Year, contains("_dev")) %>% 
  subset(Year %in% c(1970:2010)) %>% 
  melt(id.vars = c("Country", "Year"))

# levels(final_dev_pub$variable) = c(expression("eta[t]"), expression("tau[t]"), expression("b[t]"), expression("h[t]"))
...
final_dev_pub %>% 
  ...
  labs(x = "Year", y = "") +
  ggsave(file.path(loc_result, "dev_pub.png"), width = scale_graph*5, height = scale_graph*5/3)
