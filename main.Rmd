...
The discount factor $\alpha$ is set to `r 0.99^40`. The relative bargaining power of the union $\gamma$ is set to `r 0.5`. *For more details, please consult "gamma.md" file.*
...
# Remove parameters from data
data = data %>% select(names(data_base))
```
