...
I start loading all the initial script. If the summeR repository is not loaded, please download it [here](https://github.com/SummerTold/summeR). Place the summeR repository in the same folder as this repository.
...
```{r emp-ls, fig.cap="Labor share in OECD countries", warning=F}
df %>% 
...
graph_ratio_pc = redis %>% 
  ...
  scale_x_continuous(breaks = breaks_10_years, labels = labs_20_years) +
  theme_bw() +
  labs(x = element_blank(), y = element_blank(), title = "Per-capita income ratio in percentage deviation (1970 = 0)", linetype = element_blank(), color = element_blank()) +
  theme(legend.position = "right")
...
## Regressions
# (RAW) column
reg$raw = lm(formula = THETA1_log ~ k_log * Country, data = pwt2)
# (HWC) column
reg$hwc = lm(formula = THETA1_log ~ k.avh_log * Country, data = pwt2)
# (BTC) column
reg$btc = lm(formula = THETA1_log ~ k_log * Country + t_diff*Country, data = pwt2)
# (HWC-BTC) column
reg$both = lm(formula = THETA1_log ~ k.avh_log * Country + t_diff * Country, data = pwt2)
```
...
Figure \@ref(fig:graph-regime-detrend) shows raw and detrended data of the capital-per-worker and the labor-to-capital income ratio in France between 1970 and 2010. This figure corresponds to Figure 13 in the paper (appendix).
...
Loop to run regressions with changing break in the regime between 1975 and 1995.
...
split.coef = split.coef %>% 
  ...
  mutate(star = ifelse(pvalue > 0.1, NA, ifelse(pvalue > 0.05, "*", ifelse(pvalue > 0.01, "**", "***"))))
```

Prepare the table to summarize coefficients and $R^2$.

```{r regime-split-compute}
# R2
split.R2 = split.R2 %>% 
  mutate(variable = "estimate", R2 = sprintf("%.3f", R2))
...
Table \@ref(tab:tab-regime-split) summarizes the parameters for France wit a break in the regime at year $\bar{t]$. This table corresponds to Table 4 in the paper (appendix).
...
The regression with a change in the regime in 1985 has the greatest $R^2$ and all its coefficients are significant. Thus, I asssume that the break in the regime has occurred in 1985.
...
Consider the (HWC) and (HWC-BTC) specifications with the break in the regime.
...
pwt2.fr.struct = pwt2.fr %>% 
  select(Year, k.avh_log_detrend, THETA1_log_detrend)

test = efp(THETA1_log_detrend ~ k.avh_log_detrend, data = pwt2.fr.struct)
plot(test)

m0 = lm(THETA1_log_detrend ~ k.avh_log_detrend, data = pwt2.fr.struct)

bp = breakpoints(THETA1_log_detrend ~ k.avh_log_detrend, data = pwt2.fr.struct, breaks = 1)
summary(bp)
# Optimal break
pwt2.fr.struct$Year[bp$breakpoints]

m1 = lm(THETA1_log_detrend ~ k.avh_log_detrend*I(Year>1985), data = pwt2.fr.struct)

plot(bp)
lines(bp, breaks = 1)

plot(pwt2.fr.struct$k.avh_log_detrend)
...
```{r}
# Compute break point
bp = breakpoints(THETA1_log_detrend ~ k.avh_log_detrend, data = pwt2.fr, breaks = 1)
summary(bp)
# Optimal break
breakyear = pwt2.fr$Year[bp$breakpoints]
breakyear

m0 = lm(THETA1_log ~ k.avh_log + t_diff, data = pwt2.fr)
summary(m0)
m1 = lm(THETA1_log ~ k.avh_log*I(Year > breakyear) + t_diff*I(Year >= breakyear), data = pwt2.fr)
summary(m1)

waldtest(m1, m0)

# Before break point
m1a = lm(THETA1_log ~ k.avh_log, subset = Year <= breakyear, data = pwt2.fr)
summary(m1a)
# After break point
m1b = lm(THETA1_log ~ k.avh_log, subset = Year > breakyear, data = pwt2.fr)
summary(m1b)
```
...
bp = breakpoints(THETA1_log ~ k.avh_log + t_diff, data = pwt2.us, breaks = 1)
summary(bp)
# Optimal break
breakyear = pwt2.us$Year[bp$breakpoints]
breakyear

m0 = lm(THETA1_log ~ k.avh_log + t_diff, data = pwt2.us)
m1 = lm(THETA1_log ~ k.avh_log*I(Year > breakyear) + t_diff*I(Year > breakyear), data = pwt2.us)

waldtest(m1, m0)

# Before break point
m1a = lm(THETA1_log ~ k.avh_log + t_diff, subset = Year <= breakyear, data = pwt2.us)
summary(m1a)
# After break point
m1b = lm(THETA1_log ~ k.avh_log + t_diff, subset = Year > breakyear, data = pwt2.us)
summary(m1b)
```
