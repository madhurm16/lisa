...
```{r quant-init}
# Simulation periods
sim = seq(1950, 2060, 10)
# Estimation sample
est_sample = c(1950:2010)
# Graphic parameters
breaks_10_years = seq(1950, 2060, 10)
labs_20_years = as.vector(rbind(seq(1950, 2060, 20), rep("", length(breaks_10_years)/2)))
...
```{r}
# Bound beta to 0 for the US
param_base = param_base %>%
  mutate(beta = ifelse(Country == "United States" & beta < 0, 0, beta))
...
graph_dev7010 = list.sim$benchmark %>% 
  ...
  mutate_at(vars(eta, outside, w, L, u, K, Y, theta), ~ (./.[Year == 1950]-1)*100) %>% 
  subset(Year %in% c(1950:2010)) %>% 
  # Variables and names
  select(Country, Year,
  ...
  scale_x_continuous(breaks = seq(1950, 2010, 10), labels = c("1950","", "1970","", "1990", "", "2010")) +
...
graph_dev1080 = list.sim$benchmark %>% 
  ...
  geom_line() +
  facet_wrap(. ~ variable, scales = "free", nrow = 2, labeller = label_parsed) +
  scale_x_continuous(breaks = seq(2010, 2060, 10), labels = c("2010","", "2030", "", "2050", "")) +
