...
```{r Parameter - Production function - France - All, echo = FALSE}
...
```{r Parameter - Production function - France - Before, echo = FALSE}
...
```{r Parameter - Production function - France - After, echo = FALSE}
...
# Sigma
1/(1+ols.france$coefficients[2]) %>% unname()
...
est_sigma.fr = est_sigma %>% 
  subset(Country == "France") %>% 
  mutate(THETA_log.detrended = lm(THETA_log ~ t_diff) %>% residuals(),
         k_log.detrended = lm(k_log ~ t_diff) %>% residuals())

# Regression without break
ols.france_nobreak = est_sigma.fr %>% 
  lm(THETA_log.detrended ~ k_log.detrended - 1, data = .)
...
baseline %>% 
  ...
  mutate(from = ifelse(variable == "theta" & Country == "France", "model.france",
                       ifelse(variable == "theta" & Country == "United States", "model.us","data"))) %>% 
  subset(Year %in% c(1970:2020)) %>% 
  
  ggplot(aes(x = Year, y = value, color = from)) +
  geom_line() +
  ...
  scale_color_manual(breaks = c("data", "model.france", "model.us"),
                    values = c("black", brewer.pal(8, "Set1")[c(1,2)])) +
  theme_classic(base_size = 14) +
  theme(legend.position = "none", panel.spacing.x = unit(1.5, "lines")) +
  labs(x = "", y = "") +
  ggsave(file.path(loc_result, "baseline7020_facet.png"),
...
baseline %>%
  ...
  theme(legend.position = "none", legend.direction = "vertical",
        panel.spacing.x = unit(1.5, "lines")) +
  labs(x = "", y = "") +
  ggsave(file.path(loc_result, "dev_public_1080.png"), width = scale_graph*5, height = scale_graph*5/3)
...
baseline %>%
  ...
  theme(legend.position = "none", legend.direction = "vertical",
        panel.spacing.x = unit(1.5, "lines")) +
  labs(x = "", y = "") +
  ggsave(file.path(loc_result, "dev_bargain_1080.png"), width = scale_graph*5, height = scale_graph*5/3)
...
baseline %>%
  ...
  theme(legend.position = "none", legend.direction = "vertical",
        panel.spacing.x = unit(1.5, "lines")) +
  labs(x = "", y = "") +
  ggsave(file.path(loc_result, "dev_laborshare_1080.png"), width = scale_graph*5, height = scale_graph*5/3)
...
param = param %>% 
  mutate(sigma = ifelse(Country == "France", ifelse(Year <= 1980, sigma.fr_before, 1.375), sigma))
...
baseline %>% 
  ...
  theme(legend.position = "none", panel.spacing.x = unit(1.5, "lines")) +
  labs(x = "", y = "") +
  ggsave(file.path(loc_result, "baseline7010_facet2.png"),
         width = scale_graph*5, height = scale_graph*5/2)
...
```{r Graph - FillGap vs Baseline - Demo 70-10, echo = FALSE}
...
```{r Graph - FillGap vs Baseline - Demo 10-80, echo = FALSE}
...
```{r Graph - FillGap vs Baseline - Public 10-50, echo = FALSE}
...
```{r Graph - BModel - Outside 70-10, echo = FALSE}
...
```{r Graph - BModel - Outside 10-80, echo = FALSE}
...
```{r Graph - BModel - Production 70-10, echo = FALSE}
...
```{r Graph - BModel - Production 10-80, echo = FALSE}
...
decomp %>% 
  ...
  theme(legend.position = "right", legend.direction = "vertical") +
  labs(x = "Year", y = "Labor income share") +
  ggsave(file.path(loc_dev, "counter_facet_PGSR.png"), width = 1920/1080*5, height = 5)
...
decomp %>% 
  ...
  labs(x = "", y = "") +
  ggsave(file.path(loc_result, "counter_PGSR_1970.png"),
         width = scale_graph*5, height = scale_graph*5/2)
...
decomp %>% 
  ...
  theme(legend.position = "right", legend.direction = "vertical") +
  labs(x = "Year", y = "Labor income share") +
  ggsave(file.path(loc_dev, "counter_facet_DEIE.png"), width = 1920/1080*5, height = 5)
...
decomp %>% 
  ...
  scale_x_continuous(breaks = breaks_10_years, labels = labs_20_years) +
  scale_linetype_manual(name = "",
                        breaks = c("data","TDE.TIE", "TDE.FIE",
                                "FDE.TIE", "FDE.FIE"),
                        labels = c("Data", "Benchmark", expression(eta[1970]),
                                expression(N[1970]^y*", "*N[1970]^o),
                                expression(eta[1970]*", "*N[1970]^y*", "*N[1970]^o)),
                        values = c(rep("dashed", 4), "solid") %>% 
  ...
  theme(legend.position = c(0.99, 0.99),
        ...
        legend.direction = "vertical") +
  labs(x = "", y = "") +
  ggsave(file.path(loc_result, "counter_DEIE_1970.png"),
         width = scale_graph*5, height = scale_graph*5/2)
...
decomp2 %>% 
  ...
  theme(legend.direction = "vertical", legend.box = "horizontal",
        legend.position = c(0.99, 0.01), legend.justification = c("right", "bottom")) +
  labs(x = "", y = "Difference with counterfactual (in pp.)") +
  ggsave(file.path(loc_result, "decomp_PGSR_1970.png"),
         width = scale_graph*5, height = scale_graph*5/2)
...
decomp2 %>% 
  ...
  scale_fill_manual(name = "",
                    ...
                    labels = c("Direct cohort effect", "Indirect cohort effect",
                               "Interaction effect"),
                    values = brewer.pal(8, "Set1")[c(5,8,4)] %>% 
                      setNames(c("FDE.TIE", "TDE.FIE", "FDE.FIE"))) +
  theme_classic(base_size = 14) +
  theme(legend.direction = "vertical", legend.box = "horizontal",
        legend.position = c(0.99, 0.01), legend.justification = c("right", "bottom")) +
  labs(x = "", y = "Difference with counterfactual (in pp.)") +
  ggsave(file.path(loc_result, "decomp_DEIE_1970.png"),
         width = scale_graph*5, height = scale_graph*5/2)
...
decomp_mean.period = decomp2 %>% 
  ...
  summarise_at(vars(FPG.TSR.share:FDE.FIE.share), mean, na.rm = TRUE)
...
decomp_mean.period %>% 
  ...
  scale_fill_manual(name = "",
                    breaks = c("TPG.FSR.share", "FPG.TSR.share", "FPG.FSR.share",
                               "FDE.TIE.share", "TDE.FIE.share", "FDE.FIE.share"),
                    labels = c("Survival rate", "Population growth", "Interaction",
                               "Direct cohort", "Indirect cohort", "Interaction"),
                    values = brewer.pal(8, "Set1")[c(2,3,4,5,8,4)] %>% 
                      setNames(c("TPG.FSR.share", "FPG.TSR.share", "FPG.FSR.share",
                                 "FDE.TIE.share", "TDE.FIE.share", "FDE.FIE.share"))) +
  theme_classic(base_size = 14) +
  theme(legend.position = "bottom") +
  labs(x = "", y = "Percent") +
  ggsave(file.path(loc_result, "decomp_1970.png"), width = scale_graph*5, height = scale_graph*5/2)
