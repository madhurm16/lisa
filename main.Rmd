...
baseline %>% 
  ...
  theme(legend.position = "none", panel.spacing.x = unit(1.5, "lines")) +
  labs(x = "", y = "") +
  ggsave(file.path(loc_result, "baseline7020_facet2.png"),
         width = scale_graph*5, height = scale_graph*5/2)
...
baseline %>%
  ...
  ungroup() %>% 
  select(Specification, Country , Year, c("eta", "tau", "`b share`", "`h share`")) %>% 
  subset(Year %in% c(1970:2010)) %>% 
...
baseline %>%
  ...
  mutate(outside = b/(1-tau),
         ...
         "X" = X/X[Year == 1970] -1) %>% 
  ungroup() %>% 
  select(Specification, Country, Year, c("eta", "b/(1-tau)", "w", "X")) %>% 
  subset(Year %in% c(1970:2010)) %>% 
  melt(id.vars = c("Specification", "Country", "Year")) %>% 
...
baseline %>%
  ...
  mutate("YL" = Y/L,
         ...
         "theta" = theta/theta[Year == 1970] -1) %>% 
  ungroup() %>% 
  select(Specification, Country, Year, c("w", "u", "Y/L", "theta")) %>% 
  subset(Year %in% c(1970:2010)) %>% 
  melt(id.vars = c("Specification", "Country", "Year")) %>% 
