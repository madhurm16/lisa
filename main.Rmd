...
I start loading all the initial script. If the summeR repository is not loaded, please download it [here](https://github.com/SummerTold/summeR). Place the summeR repository in the same directory as this one.
...
Then, I load the data. For more details on data, see the README.md file in _data/_raw. For more details on data treatment and the construction of these datasets, see the scripts in _script/_treatment.
...
```{r emp-ls, fig.cap="Labor share in OECD countries"}
df %>% 
...
Figure \@ref(fig:emp-lsadj) presents the labor share data on the same sample of countries over the same period with different adjustment methods for self-employed income from the PWT. The labor share from the OECD data is also plotted. Although the magnitude changes according to the adjustment method and the data source, the dynamics remain roughly similar.
...
```{r emp-lsdepcor, fig.cap="Correlation between the old-age dependency ratio and the labor share in OECD countries"}
require(ggpmisc)
df %>% 
  subset(Year %in% c(1970:2010)) %>% 
  select(Country, Year, lab_sh1, i_lab_sh1, dep) %>% 
...
head(data_base, 10)
```
Compute the young and old population sizes such that $N^y_t = n_t \times N^y_{t-1}$ and $N^o_t = p_t \times N^y_{t-1}$. The young population at previous period corresponds to the young population 40 years before.
...
Figure \@ref(tab:tab-param) summarizes the parameters for both countries. This table corresponds to Table 1 in the paper.
...
graph_baseline7010_LS +
    scale_color_brewer(palette = "Set1") +
  ggsave(file.path(loc_graphic, "quant-base7010-color.png"), width = 7, height = 7/1.5)
...
graph_baseline1080_LS +
  ...
  guides(color = "none") +
  ggsave(file.path(loc_graphic, "quant-base1080-color.png"),  width = 7, height = 7/1.5)
```
...
tab_demo_1970 = list.sim$baseline %>% 
  ...
  mutate(
    ...
    "Variable" = c("Survival rate", "Population growth",
                        "Expected survival rate", "Old-age-dependency ratio",
                        "Young political power")) %>% 
...
```{r}
# Dataframe for decomposition
decomp2P1 = decompP1 %>% subset(from == "sim") %>% 
  mutate(Spe_PGSR = interaction(PG, SR), Spe_DEIE = interaction(DE, IE)) %>% 
  select(Country, Year, break_year, Spe_PGSR, Spe_DEIE, theta) %>% setDT %>% 
  dcast(Country + Year + break_year ~ Spe_PGSR + Spe_DEIE, value.var = "theta") %>% 
  setNames(c("Country", "Year", "break_year",
...
pgsr_labels = c("Data", "Benchmark", "Constant population growth", "Constant survival rate",
               "Baseline")
...
graph_pgsr_decomp = decomp2 %>%
  ...
  labs(x = element_blank(), 
       y = "Difference with the baseline counterfactual (in pp.)", 
       fill = element_blank(),
       title = element_blank())
...
# Paper version
graph_pgsr_counterP1 +
  scale_linetype_manual(labels = pgsr_labels, 
                        values = c("solid", "solid", "dashed", "twodash", "dotted")) +
  scale_color_manual(labels = pgsr_labels, values = c("grey", rep("black", 4))) +
  ggsave(file.path(loc_graphic, "quant-pgsr-counterP1.png"), width = 7, height = 7/1.5)
...
graph_pgsr_counterP1 +
  ...
  scale_color_manual(labels = pgsr_labels, 
                     values = c("black", brewer.pal(8, "Set1")[c(1:4)])) +
  ggsave(file.path(loc_graphic, "quant-pgsr-counterP1-color.png"), width = 7, height = 7/1.5)
```

```{r}
## PGSR Decomposition
graph_pgsr_decompP1 = decomp2P1 %>%
  subset(variable %in% c("FPG.TSR", "TPG.FSR", "FPG.FSR")) %>%
  mutate(variable = factor(variable, levels = c("FPG.TSR", "TPG.FSR", "FPG.FSR"))) %>%
...
graph_pgsr_decompP1 +
  geom_line(aes(y = position_PGSR), color = "black") +
  scale_fill_grey(labels = pgsr_labels2, start = .1, end = .8) +
  ggsave(file.path(loc_graphic, "quant-pgsr-decompP1.png"), width = 7, height = 7/1.5)

# Color version
graph_pgsr_decompP1 + 
  geom_line(aes(y = position_PGSR), color = brewer.pal(8, "Set1")[1]) +
  scale_fill_manual(labels = pgsr_labels2, values = brewer.pal(8, "Set1")[c(2:4)]) +
  ggsave(file.path(loc_graphic, "quant-pgsr-decompP1-color.png"), width = 7, height = 7/1.5)
```
...
```{r}
# Counterfactual graphic parameters
deie_labels = c("Data", "Benchmark", "Factor accumulation", "Policy mechanism",
                "Baseline")
...
# Paper version
graph_deie_counterP1 +
    scale_linetype_manual(labels = deie_labels, 
                        values = c("solid", "solid", "dashed", "twodash", "dotted")) +
    scale_color_manual(labels = deie_labels, values = c("grey", rep("black", 4))) +
  ggsave(file.path(loc_graphic, "quant-deie-counterP1.png"), width = 7, height = 7/1.5)
...
graph_deie_counterP1 +
    ...
    scale_color_manual(labels = deie_labels, 
                       values = c("black", brewer.pal(8, "Set1")[c(1,5,8,4)])) +
  ggsave(file.path(loc_graphic, "quant-deie-counterP1-color.png"), width = 7, height = 7/1.5)
```

```{r}
## DEIE Decomposition
graph_deie_decompP1 = decomp2P1 %>% 
  subset(variable %in% c("FDE.TIE", "TDE.FIE", "FDE.FIE")) %>% 
  mutate(variable = factor(variable, levels = c("FDE.TIE", "TDE.FIE", "FDE.FIE"))) %>% 
...
graph_deie_decompP1 +
  geom_line(aes(y = position_DEIE), color = "black") +
    scale_fill_grey(labels = deie_labels2, start = .1, end = .8) +
  ggsave(file.path(loc_graphic, "quant-deie-decompP1.png"), width = 7, height = 7/1.5)

# Color version
graph_deie_decompP1 + 
  geom_line(aes(y = position_DEIE), color = brewer.pal(8, "Set1")[1]) +
  scale_fill_manual(labels = deie_labels2, values = brewer.pal(8, "Set1")[c(5,8,4)]) +
  ggsave(file.path(loc_graphic, "quant-deie-decompP1-color.png"),
         width = 7, height = 7/1.5)
...
```{r }
# Compute the share of each effect/channel for two sub_periods 1980-2010 & 2020-2050
decomp_mean.periodP1 = decomp2P1 %>% select(-contains("position")) %>% setDT %>% 
  dcast(Country + Year + break_year ~ variable) %>% 
  mutate(Country_break = interaction(break_year, Country, sep = " - "),
         Period = ifelse(Year %in% c(1970:2010), "P1",
                         ifelse(Year %in% c(2020:2050), "P2", "P3"))) %>% 
...
```{r}
# Summary
graph_decomp_summaryP1 = decomp_mean.periodP1 %>% setDT %>% 
  melt(id.vars = c("Country_break", "Period")) %>% 
  mutate(value = value*100) %>% 
  subset(Country_break %in% c("1970 - France", "1970 - United States")) %>% 
  mutate(Country = ifelse(Country_break == "1970 - France", "France", "United States"),
...
# Paper version
graph_decomp_summaryP1 +
  geom_bar(stat = "identity", position = position_dodge(), color = "black") +
  scale_fill_grey(start = 1, end = 0) +
  ggsave(file.path(loc_graphic, "quant-sum-decompP1.png"), width = 7, height = 7/1.5)

# Color version
graph_decomp_summaryP1 +
    geom_bar(stat = "identity", position = position_dodge(), alpha = .7) +
    scale_fill_manual(values = brewer.pal(8, "Set1")[c(2,3,5,8,4)]) +
  ggsave(file.path(loc_graphic, "quant-sum-decompP1-color.png"), 
         width = 7, height = 7/1.5)
```
...
graph_dev_income_ratio_capita = redis %>% 
  ...
  theme_bw() +
  labs(x = element_blank(), y = element_blank(), 
       title = "Per-capita income ratio in percentage deviation (1970 = 0)",
       linetype = element_blank(), color = element_blank()) +
  theme(legend.position = "right")
...
retage_p  %>% 
  ...
  theme(legend.position = "right", legend.text.align = 0, 
        panel.spacing.x = unit(1, "lines")) +
  labs(x = element_blank(), y = element_blank(), linetype = "Survival rate",
       color = "Scenario", title = element_blank()) +
...
retage_ls %>% 
  ...
  labs(x = element_blank(), y = "Labor share", linetype = "Scenario",
       color = "Scenario", title = element_blank()) +
...
result_after = result1 %>% 
  subset(inter > as.numeric(k2))
result_before = result1 %>% 
  subset(inter <= as.numeric(k2))
...
graph_detrend +
  scale_color_grey(start = 0, end = 0,
                   labels = c("Normalized capital-per-worker (in log)",
                              "Labor-to-capital income ratio (in log)")) +
  scale_linetype_manual(values = c("solid", "dashed"),
                        labels = c("Normalized capital-per-worker (in log)",
                                   "Labor-to-capital income ratio (in log)")) +
...
graph_regline = pwt2.fr %>% 
  ...
  facet_wrap(type ~ ., scale = "free",
    labeller = labeller(type = c(raw = "Raw data", detrend = "Detrended data"))) +
  theme_bw() +
  labs(x = "Normalized capital-per-worker (in log)", 
       y = "Labor-to-capital income ratio (in log)") +
  theme(legend.position = "bottom")
