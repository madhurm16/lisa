...
### Demography break year

```{r Demography in 1960, echo = FALSE}

breakers = demo %>% 
  subset(Year == 1960) %>% 
  mutate(Ny = NA, No = NA, p1 = NA) %>% 
  select(Country, Year, Ny, No, n, p, p1) %>% 
  rbind(., data_base %>% 
          select(Country, Year, Ny, No, n, p, p1) %>% 
          subset(Year == 2000) %>% 
          ungroup()) %>% 
  ...
  mutate(eta = n/p*(1+alpha*p1)/omega,
         Ny = ifelse(Year == 2010, Ny[Year == 1970]*n, Ny),
         No = ifelse(Year == 2010, Ny*p/n, No)) %>% 
  select(-alpha, -omega)
...
```{r Graph CModel - Explanation Power - Pie chart, echo = FALSE}
...
decomp_mean.period %>% 
  ...
  subset(Country_break %in% c("1970 - France", "1970 - United States")) %>% 
  mutate(Country = ifelse(Country_break == "1970 - France", "France", "United States"),
         effect = ifelse(variable %>% grepl(pattern = "PG", .), "determinant", "channel")) %>% 
  
  ggplot(aes(x = "", y = value, fill = variable)) +
  geom_bar(stat = "identity", alpha = .5) +
  facet_wrap(interaction(Country, Period, effect) ~ ., ncol = 4) +
...
```{r Retirement age - 2010 break_year - Specification 1, include = FALSE}
...
for(ret_spe in c(1,2)){
  ...
  merge(param) %>% 
  # Demography
  demo_changer(break_year = break_year, p1_equals_p_fix = TRUE, SR = "FSR") %>% 
  # Define specification model
  mutate(Specification = spe) %>%
  #Reorder variables
  select(Specification, everything())
