...
ols_sigma = df %>% 
  ...
  ungroup() %>% 
  lm(k_log ~ Country + THETA_log_neg*Country + Year*Country - THETA_log_neg - Year, data = .)
...
baseline %>%
  ...
  mutate(outside = b/(1-tau),
         ...
         "w" = w/w[Year == 1970] -1) %>% 
  select(Country, Year, c("eta", "b/(1-tau)", "w")) %>% 
  subset(Year %in% c(1970:2010)) %>% 
  melt(id.vars = c("Country", "Year")) %>% 
...
param = param %>% 
  subset(Country == "France") %>% 
  mutate(sigma = ifelse(Year <= 1980, 0.98, sigma))
```
...
baseline %>%
  ...
  mutate(outside = b/(1-tau),
         ...
         "b/(1-tau)" = outside/outside[Year == 1970] -1,
         "w" = w/w[Year == 1970] -1) %>% 
  select(Specification, Year, c("eta", "b/(1-tau)", "w")) %>% 
  subset(Year %in% c(1970:2010)) %>% 
  melt(id.vars = c("Specification", "Year")) %>% 
