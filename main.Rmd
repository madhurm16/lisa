...
# France
ols.france = est_sigma %>% 
  subset(Country == "France") %>% 
  lm(THETA_log_neg ~ k_log + t_diff, data = .)

# Result visualization
summary(ols.france)

# Sigma
1/(1+ols.france %>% .$coefficients %>% .["k_log"] %>% unname())


```

Biased Technical Change significant at a 10% level. Suspect a break point. Best break point : 1986.

```{r Parameter - Production function - France, echo = FALSE}
# Detrend data : in order to assume a constant biased technical change
est_sigma.fr = est_sigma %>% 
  subset(Country == "France") %>% 
  mutate(THETA_log_neg.detrended = lm(THETA_log_neg ~ t_diff) %>% residuals(),
         k_log.detrended = lm(k_log ~ t_diff) %>% residuals())

# Regression without break
ols.france_nobreak = est_sigma.fr %>% 
  lm(THETA_log_neg.detrended ~ k_log.detrended - 1, data = .)
summary(ols.france_nobreak)
```
...
for(splitter in c(1975:1995)){
  ...
  est_sigma.fr = est_sigma.fr %>% 
    mutate(after_split = ifelse(Year >= splitter, 1, 0),
           before_split = ifelse(Year < splitter, 1, 0))
  
  # Break
  ols.france_dummy = est_sigma.fr %>% 
    lm(THETA_log_neg.detrended ~ k_log.detrended*before_split + k_log.detrended*after_split - 1 - k_log.detrended,
       data = .)
...
```
 The Year 1986 is the best to split the sample
```{r}
...
# Before
ols.france_before = est_sigma.fr %>% 
  subset(Year < splitter) %>% 
  lm(THETA_log_neg ~ k_log + t_diff, data = .)
summary(ols.france_before)

# After
ols.france_after = est_sigma.fr %>% 
  subset(Year >= splitter) %>% 
  lm(THETA_log_neg ~ k_log + t_diff, data = .)
summary(ols.france_after)

```
No biased technical change anymore if I split the estimation in two regimes
...
ols.france_after = est_sigma.fr %>% 
  subset(Year >= splitter) %>% 
  lm(THETA_log_neg.detrended ~ k_log.detrended, data = .)
summary(ols.france_after)
...
# United States with BTC
ols.us = est_sigma %>% 
  subset(Country == "United States") %>% 
  lm(THETA_log_neg ~ k_log+ t_diff, data = .)

# Result visualization
summary(ols.us) # BTC is not significant

```

Biased Technical Change is not significant.

```{r Parameter - Production function - United States, echo = FALSE}
# New estimate, without BTC
ols.us.no_btc = est_sigma %>% 
  subset(Country == "United States") %>% 
  lm(THETA_log_neg ~ k_log, data = .)

# Result visualization
summary(ols.us.no_btc)

# Sigma
1/(1+ols.us.no_btc %>% .$coefficients %>% .["k_log"] %>% unname())

```

#### Summary

```{r Parameter - Production function - Display, echo = FALSE}
...
param_base = param_base %>%
  mutate("phi" = rep(1 - df$theta[df$Year == 1970], each = length(sim)), 
         ...
         "a" = rep(ols_sigma$coefficients[c((2*length(country_set)+1):(3*length(country_set)))]/
                     ols_sigma$coefficients[c((length(country_set)+1):(2*length(country_set)))],
                   each = length(sim)))
...
# Break year in sigma for France
# param = param %>% 
#    mutate(sigma = ifelse(Country == "France", ifelse(Year > 1980, sigma, 1.01), sigma))
...
```{r Graph - BModel - Public 70-10, echo = FALSE}
...
baseline %>%
  ...
  select(Country, Year, c("eta", "b", "h", "tau")) %>% 
  subset(Year %in% c(1970:2010)) %>% 
  melt(id.vars = c("Country", "Year")) %>% 
  ...
  labs(x = "", y = "") +
  ggsave(file.path(loc_result, "dev_public_7010.png"), width = scale_graph*5, height = scale_graph*5/3)
  
```

```{r Graph - BModel - Bargaining 70-10, echo = FALSE}
...
baseline %>%
  ...
  mutate(outside = b/(1-tau),
         ...
         "w" = w/w[Year == 1970] -1,
         "X" = X/X[Year == 1970] -1) %>% 
  select(Country, Year, c("eta", "b/(1-tau)", "w", "X")) %>% 
  subset(Year %in% c(1970:2010)) %>% 
  melt(id.vars = c("Country", "Year")) %>% 
  ...
  theme(legend.position = "none", legend.direction = "vertical",
        panel.spacing.x = unit(1.5, "lines")) +
  labs(x = "", y = "") +
  ggsave(file.path(loc_result, "dev_bargain_7010.png"), width = scale_graph*5, height = scale_graph*5/3)
  
```

```{r Graph - BModel - Labor Share 70-10, echo = FALSE}
...
baseline %>%
  ...
  select(Country, Year, c("w", "u", "Y/L", "theta")) %>% 
  subset(Year %in% c(1970:2010)) %>% 
  melt(id.vars = c("Country", "Year")) %>% 
  ...
  theme(legend.position = "none", legend.direction = "vertical",
        panel.spacing.x = unit(1.5, "lines")) +
  labs(x = "", y = "") +
  ggsave(file.path(loc_result, "dev_laborshare_7010.png"), width = scale_graph*5, height = scale_graph*5/3)
...
```{r Graph - BModel - Public 10-50, echo = FALSE}
...
baseline %>%
  ...
  facet_wrap(variable ~ ., scales = "fixed", nrow = 1, labeller = label_parsed) +
  scale_color_manual(values = brewer.pal(8, "Set1")) +
  scale_x_continuous(labels = c("2010", "", "2030", "", "2050")) +
  theme_classic(base_size = 14) +
  theme(legend.position = "none", legend.direction = "vertical",
        panel.spacing.x = unit(1.5, "lines")) +
  labs(x = "", y = "") +
  ggsave(file.path(loc_result, "dev_public_1050.png"), width = scale_graph*5, height = scale_graph*5/3)
  
```


```{r Graph - BModel - Bargaining 10-50, echo = FALSE}
...
baseline %>%
  ...
  theme(legend.position = "none", legend.direction = "vertical",
        panel.spacing.x = unit(1.5, "lines")) +
  labs(x = "", y = "") +
  ggsave(file.path(loc_result, "dev_bargain_1050.png"), width = scale_graph*5, height = scale_graph*5/3)
  
```

```{r Graph - BModel - Labor Share 10-50, echo = FALSE}
...
baseline %>%
  ...
  theme(legend.position = "none", legend.direction = "vertical",
        panel.spacing.x = unit(1.5, "lines")) +
  labs(x = "", y = "") +
  ggsave(file.path(loc_result, "dev_laborshare_1050.png"), width = scale_graph*5, height = scale_graph*5/3)

```

### Fill the gap : France 1980

```{r Fill the gap France 1980 - Parameter, echo = FALSE}
# Change param for France
param = param %>% 
  mutate(sigma = ifelse(Country == "France", ifelse(Year <= 1980, 1, sigma), sigma))
# 
# # Change A
# data = data_base %>% 
#   # No biased technical change
...
```{r Fill the gap France 1980 - Simulation, echo = FALSE}
...
```{r Graph - FGapModel Labor Share - Plot 1970-2020, echo = FALSE}
...
fillgap %>% 
  ...
  subset(Country == "France") %>% 
  melt(id.vars = c("Country", "Year", "Specification")) %>% 
  subset(is.na(value) == FALSE) %>% 
  mutate(Specification = ifelse(variable == "labsh", NA, Specification)) %>%
  subset(Year %in% c(1970:2010)) %>% 
  # mutate(value = ifelse(is.na(Specification) == FALSE, round(value, 2), value)) %>% 
  unique() %>% 
  ...
  scale_color_manual(name = "Specification",
                       breaks = c("baseline", "fillgap"),
                       labels = c("Baseline", "Fill Gap"),
                       values = c("black", brewer.pal(8, "Set1")[1]),
                       na.value = "black") +
  theme_classic(base_size = 14) +
  theme(legend.position = "none") +
  labs(x = "Year", y = "Labor income share") +
  ggsave(file.path(loc_result, "fillgap.png"), width = scale_graph*5, height = scale_graph*5/2)

```

#### Variables

```{r Graph - FillGap vs Baseline - Public 70-10, echo = FALSE}
...
baseline %>%
  ...
  mutate(
    # Deviation from the 1970 steady state
         ...
         "h" = h/h[Year == 1970] -1,
         "tau" = tau/tau[Year == 1970] -1) %>% 
  select(Specification, Year, c("eta", "b", "h", "tau")) %>% 
  subset(Year %in% c(1970:2010)) %>% 
  melt(id.vars = c("Specification", "Year")) %>% 
  
  ggplot(aes(x = Year, y = value, linetype = Specification)) +
  geom_line(color = brewer.pal(8, "Set1")[1]) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  facet_wrap(variable ~ ., scales = "fixed", nrow = 1, labeller = label_parsed) +
  scale_x_continuous(labels = c("1970", "", "1990", "", "2010")) +
  ...
  theme(legend.position = "none", legend.direction = "vertical",
        panel.spacing.x = unit(1.5, "lines")) +
  labs(x = "", y = "") +
  ggsave(file.path(loc_result, "fillvsbase_dev_public_7010.png"), width = scale_graph*5, height = scale_graph*5/3)
...
baseline %>%
  ...
  mutate(outside = b/(1-tau),
         ...
         "w" = w/w[Year == 1970] -1,
         "X" = X/X[Year == 1970] -1) %>% 
  select(Specification, Year, c("eta", "b/(1-tau)", "w", "X")) %>% 
  subset(Year %in% c(1970:2010)) %>% 
  melt(id.vars = c("Specification", "Year")) %>% 
  ...
  theme(legend.position = "none", legend.direction = "vertical",
        panel.spacing.x = unit(1.5, "lines")) +
  labs(x = "", y = "") +
  ggsave(file.path(loc_result, "fillvsbase_dev_bargain_7010.png"), width = scale_graph*5, height = scale_graph*5/3)
...
baseline %>%
  ...
  mutate("YL" = Y/L,
         ...
         "Y/L" = YL/YL[Year == 1970] -1,
         "theta" = theta/theta[Year == 1970] -1) %>% 
  select(Specification, Year, c("w", "u", "Y/L", "theta")) %>% 
  subset(Year %in% c(1970:2010)) %>% 
  melt(id.vars = c("Specification", "Year")) %>% 
  ...
  theme(legend.position = "none", legend.direction = "vertical",
        panel.spacing.x = unit(1.5, "lines")) +
  labs(x = "", y = "") +
  ggsave(file.path(loc_result, "fillvsbase_dev_laborshare_7010.png"), width = scale_graph*5, height = scale_graph*5/3)
