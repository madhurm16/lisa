...
The purpose of this section is to investigate whether there is a break in the estimated elasticity of substitution. In particular for France. Indeed, there capital-per-worker $k$ grows at a relatively constant rate while the labor share $\\theta$ faces a huge decrease during the 80's.
...
There is a biased technical change, which is constant and significant. I have to keep the same rate of technical change. Because if I split the sample, I would obtain two different BTC growth rates. Thus, a way is to detrend the capital-per-worker.
...
robust.summary(ols_base_detrend)  
...
ols_before = pwt_break %>% 
  subset(Year <= spliter) %>% 
  lm(k_nor_log_detrend ~ THETA_log_neg_detrend, data = .)
robust.summary(ols_before)

# After spliter
ols_after = pwt_break %>% 
  subset(Year > spliter) %>% 
  mutate(t_diff = Year - spliter) %>% 
  lm(k_nor_log_detrend ~ THETA_log_neg_detrend, data = .)
robust.summary(ols_after)
