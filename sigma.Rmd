...
pwt_break %>% 
  ...
  scale_x_continuous(breaks = c(1970, 1983, 1985, 1989, 2000, 2010)) +
  theme_classic()
...
df_break = df_break %>% 
  setNames(c("splitter", "sigma_before", "p_before", "sigma_after", "p_after")) %>% 
  subset(p_before <= p_baseline & p_after <= p_baseline)
...
df_break = df_break %>% merge(cwed, by.x = "splitter", by.y = "Year") %>% 
  mutate(xcdt_ok = ifelse(sigma_after < sigma_bar, TRUE, FALSE))
...
Hence, only the break in 1982 complies with the condition to ensure an equilibrium with positive wage and capital-to-labor ratio. Thus, instead of using a unique capital-labor elasticity of substitution of 1.356, I can consider that its value is **0.685** for 1970 and 1980, then **2.283**.
