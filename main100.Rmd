...
# Country set
CountrySet1950 = c("France", "United States")
CountrySet1960 = "Australia"
CountrySet1970 = "Canada"
CountrySet1980 = c("Japan", "Netherlands", "Italy")
CountrySetAll = c(CountrySet1950, CountrySet1960, CountrySet1970, CountrySet1980)
# Year dataframe
df.FirstYear = data.frame(Country = CountrySetAll, FirstYear = c(1950, 1950, 1960, 1970, rep(1980, 3)))
# List
list.graph = list()
# Graphic paramdeters
breaks_10_years = seq(1950, 2100, 10)
labs_20_years = as.vector(rbind(seq(1950, 2100, 20), rep("", length(breaks_10_years)/2)))
```
...
pwt = read.csv(file.path(loc_data, "_final", "pwt100.csv"))
# OECD data
...
# Van Vliet Caminada (Unemployment replacement rate)
vvc = read.csv(file.path(loc_data, "_final", "vvc.csv"))
## Merge
...
# Base version
list.graph$emp$ls$base = df %>% 
  subset(Year %in% c(1950:2019)) %>% 
  select(Country, Year, lab_sh1, i_lab_sh1) %>% 
  subset(Country %in% c(CountrySet1950)) %>%
  mutate(lab_sh1 = ifelse(i_lab_sh1 == 1, lab_sh1, NA)) # Remove interpolated labor share by the PWT

# Paper version
list.graph$emp$ls$base %>% 
  
  ggplot(aes(x = Year, y = lab_sh1, shape = Country)) +
  geom_point(alpha = .7) +
  geom_line(alpha = 0.7) +
  scale_color_brewer(palette = "Set1") +
  scale_shape_manual(values = c(15:18, 1:6)) +
  scale_x_continuous(breaks = seq(1950, 2020, 10)) +
  labs(x = "Year", y = element_blank(), title = "Labor share", 
       color = "Country", shape = "Country") +
  theme_bw() +
  theme(legend.position = "right")
  ggsave(file.path(loc_graphic, "emp-ls-paper.png"), width = 7, height = 7/2)

# Color version
list.graph$emp$ls$base %>% 
  
  ggplot(aes(x = Year, y = lab_sh1, color = Country, shape = Country)) +
  ...
  theme_bw() +
  theme(legend.position = "right")
  ggsave(file.path(loc_graphic, "emp-ls-color.png"), width = 7, height = 7/2)
```
...
df %>% 
  ...
  subset(Country %in% CountrySetAll) %>%
...
pwt %>% 
  ...
  mutate(variable = factor(variable,
    labels = c("Mixed income", "Part mixed income", "Average wage", "Agriculture"))) %>% 
...
```{r}
# Boomer period
boomer_period = data.frame(
  start = c(1946, 1946+20, 1946+60),
  end = c(1964+20, 1964+60, 2050),
  period = c("Child (0-20)", "Young  (20-60)", "Old (60+)") %>% 
    factor(., levels = .))
```


```{r}
list.graph$demo$boomer$data = demo %>% 
  subset(Year %in% c(1950:2050)) %>%
  mutate(young_shr = young/total,
         child_shr = child/total,
         old_shr = old/total) %>% 
  select(Country, Year, young_shr, child_shr, old_shr) %>% 
  subset(Country %in% c(CountrySet1950)) %>% 
  setDT %>% melt(id.vars = c("Country", "Year")) %>%
  mutate(variable = factor(variable, levels = c("child_shr", "young_shr", "old_shr"),
                           labels = c("Child (<20)", "Young (20-60)", "Old (60+)")))

# list.graph$demo$boomer$color = 
  list.graph$demo$boomer$data %>% 
  
  ggplot(aes(x = Year, y = value, shape = variable, linetype = variable)) +
  geom_rect(data = boomer_period, inherit.aes = FALSE,
            aes(xmin = start, xmax = end, ymin = -Inf, ymax = Inf, fill = period),
            color = "transparent", alpha = .2) +
  geom_point(data = subset(list.graph$demo$boomer$data, Year %in% seq(1950, 2050, 10)),
             alpha = .7) +
  geom_line(alpha = .7) +
  facet_wrap(Country ~ .) +
  ...
  labs(x = "Year", y = element_blank(), 
       title = "Share of population groups (in percent of the total population)", 
       shape = "Population group", fill = "Boomers' period", linetype = "Population group") +
  guides(fill = guide_legend(order = 3), shape = guide_legend(order = 1), linetype = guide_legend(order = 1)) +
  theme_bw() + theme(legend.position = "right")
  ggsave(file.path(loc_graphic, "emp-boom-color.png"), width = 7, height = 7/2)


# list.graph$demo$boomer$paper = 
  list.graph$demo$boomer$data %>% 
  ...
  guides(fill = guide_legend(order = 3), shape = guide_legend(order = 1), linetype = guide_legend(order = 1)) +
  theme_bw() + theme(legend.position = "right")
  ggsave(file.path(loc_graphic, "emp-boom-paper.png"), width = 7, height = 7/2)
```
...
list.graph$emp$ls$base %>% 
  ...
  ggplot(aes(x = Year, y = lab_sh1, color = Country)) +
  geom_rect(data = boomer_period, inherit.aes = FALSE,
            aes(xmin = start, xmax = end, ymin = -Inf, ymax = Inf, fill = period),
            color = "transparent", alpha = .2) +
  geom_point(data = subset(list.graph$emp$ls$base, Year %in% seq(1950, 2050, 10)),
             alpha = .7) +
  geom_line(alpha = .7) +
  facet_wrap(Country ~ .) +
  scale_fill_brewer(palette = "Set1") +
  ...
  labs(x = "Year", y = element_blank(), title = "Labor share") +
  theme_bw() + theme(legend.position = "right")
  ggsave(file.path(loc_graphic, "emp-boomLS-color.png"), width = 7, height = 7/2)
...
df %>% 
  ...
  geom_point(aes(color = Country, shape = Country), alpha = .7) +
  ...
  scale_shape_manual(values = c(15:18, 1:6)) +
  scale_color_brewer(palette = "Set1") +
  labs(x = "Old-age-dependency ratio", y = "Labor share", 
...
```{r}
lm(lab_sh1 ~ dep + Country + Year, data = df, subset = i_lab_sh1 == 1 & Country %in% CountrySetAll) %>% 
  summary()
```

```{r}
boomer_period %>% 
  mutate_at(vars(start, end), ~ ifelse(.<1970, 1970, .))

# test = 
  df %>% 
  select(Country, Year, URR = UC1000) %>% 
  subset(Country %in% CountrySet1950) %>% 
  subset(!is.na(URR)) %>% 
  setDT %>% melt(id.vars = c("Country", "Year")) %>% 
  
  ggplot(aes(x = Year, y = value, color = Country, linetype = variable)) +
  geom_line() +
  geom_rect(data = boomer_period2, inherit.aes = FALSE,
            aes(xmin = start, xmax = end, ymin = -Inf, ymax = Inf, fill = period),
            color = "transparent", alpha = .2) +
  scale_x_continuous(breaks = seq(1950, 2050, 10), limits = c(1970, NA)) +
  theme_bw()
...
boomer_year = data.frame(Year = c(1946:2050),
           Boomer = c(rep("Child", 1966-1946+1), 
           rep("Child + Young", 1984-1966),
           rep("Young", 2006-1984), 
           rep("Young + Old", 2024-2006),
           rep("Old", 2050-2024)))

lm(UC1000 ~ Boomer + Country + Year, data = merge(df, boomer_year, by = "Year"), subset = Country %in% CountrySetAll) %>% 
  screenreg()
...
pwt0 = pwt %>%
  ...
  subset(Country %in% CountrySetAll & Year >= 1950)
# OECD data
oecd0 = oecd %>% 
  select("Country", "Year", tax_rate = "tax_rev_PC_GDP") %>% 
  subset(Country %in% CountrySetAll & Year >= 1950) %>% 
  # Take first tax rate when NA
  merge(pwt0, ., by = c("Country", "Year"), all.x = T) %>% 
  group_by(Country) %>% 
  mutate(tax_rate = ifelse(is.na(tax_rate), first(na.omit(tax_rate)), tax_rate)) %>% 
  ungroup
# VVC data
vvc0 = vvc %>% 
  select("Country", "Year", URRFamily) %>% 
  subset(Country %in% CountrySetAll & Year >= 1950)
# Demographic data
demo0 = demo %>% 
  ...
  subset(Country %in% CountrySetAll)
## Merge
df0 = merge(pwt0, oecd0, all = T) %>% merge(demo0, all = T) %>% merge(vvc0, all = T) %>% 
  subset(Year >= 1950) %>%  
  merge(df.FirstYear, by = "Country") %>% 
  subset(Year >= FirstYear) %>% 
  subset(Country %in% CountrySet1950)
```
...
df1 = df0 %>%
  group_by(Country) %>% 
  mutate(
    Ny = young/first(young),
    u = 1 - emp*1000/young_1564,
    L = Ny*(1-u),
  ...
  select(Country, Year, FirstYear, Ny, No, n, p, p1, dep, K, L, k, theta, tau, u) %>% 
  ungroup() %>% 
...
#          # X = URRFamily, # Unemployment replacement rate
#          ) %>% 
#   select(Country, Year, FirstYear, Ny = young, No = old, n, p, p1, dep, K, L, Y, theta, tau, u) %>% 
...
data_base = df1 %>% 
  ...
  subset(Year %in% seq(1950, 2100, 10)) %>% 
  # Select predetermined variables of the 1st sequence
  select(Country, Year, FirstYear, Ny, No, n, p, p1, K) %>% 
  # Define Sequence and Period
  mutate(Sequence = ((Year - FirstYear)/10) %% 4 + 1) %>% 
  group_by(Country, Sequence) %>% 
  mutate(Period = row_number()) %>% ungroup %>% 
  # Remove values for Ny, No and K after the 1st periods
  mutate_at(vars("Ny", "No", "K"), ~ ifelse(Period > 1, NA, .)) %>% 
  # Create empty variables
  mutate(
    eta = NA, AK = NA, AL = NA, k1 = NA, k2 = NA, k = NA, X = NA, L = NA, w = NA,
    r = NA, Y = NA, u = NA, theta = NA, tau = NA, b = NA, h = NA, S = NA) %>% 
  # Mean of p1 growth for p1 between 2060 and 2100
  group_by(Country) %>% 
  merge(., mutate(., n_row = row_number(), n_row_last = last(n_row)) %>% 
          subset(n_row > n_row_last-3) %>% 
          summarise(p1.growth = mean(p/lag(p)-1, na.rm = T)), by = "Country") %>% 
  merge(., subset(., Year == 2060) %>% select(Country, p1.2060 = p1), by = "Country") %>% 
  mutate(p1 = ifelse(Year > 2060, p1.2060*(1+p1.growth)^({Year-2060}/10), p1)) %>% 
  ungroup
# Re-order variables
data_base = data_base %>% 
  select(Country, Year, FirstYear, Sequence, Period, Ny, No, n, p, p1, eta, AK, AL,
...
# Variable from PWT
pwt1 = pwt %>%
  subset(i_lab_sh1 == 1 & Country %in% CountrySetAll) %>% 
  group_by(Country) %>% 
  mutate(L = emp * 1000, # Labor
         K = rnna * 1000, # Capital
...
#
list.reg = list()
df.sigma = data.frame(Country = CountrySetAll, raw = NA, hwc = NA, btc = NA, both = NA)

for(country in CountrySetAll){
  
  list.reg$sigma[[country]][["raw"]] = lm(THETA1_log ~ k_log, data = pwt2, subset = Country == country)
  list.reg$sigma[[country]][["hwc"]] = lm(THETA1_log ~ k.avh_log, data = pwt2, subset = Country == country)
  list.reg$sigma[[country]][["btc"]] = lm(THETA1_log ~ k_log + t_diff, data = pwt2, subset = Country == country)
  list.reg$sigma[[country]][["both"]] = lm(THETA1_log ~ k.avh_log + t_diff, data = pwt2, subset = Country == country)
  
  df.sigma$raw[df.sigma$Country == country] = coef(list.reg$sigma[[country]][["raw"]])["k_log"] %>% 
    {1/(1+.)} %>% unname
  df.sigma$hwc[df.sigma$Country == country] = coef(list.reg$sigma[[country]][["hwc"]])["k.avh_log"] %>% 
    {1/(1+.)} %>% unname
  df.sigma$btc[df.sigma$Country == country] = coef(list.reg$sigma[[country]][["btc"]])["k_log"] %>% 
    {1/(1+.)} %>% unname
  df.sigma$both[df.sigma$Country == country] = coef(list.reg$sigma[[country]][["both"]])["k.avh_log"] %>% 
    {1/(1+.)} %>% unname
}
...
screenreg(list.reg$sigma$`United States`, stars = c(0.01, 0.05, 0.1), custom.coef.map = coef.map, digits = 3)
...
#     before85 = lm(THETA_log ~ k_log, data = df1.sigma0,
...
# From appendix
df.sigma$final[df.sigma$Country=="France"] = df.sigma$both[df.sigma$Country=="France"]
df.sigma$final[df.sigma$Country=="United States"] = df.sigma$hwc[df.sigma$Country=="United States"]
df.sigma$final[df.sigma$Country=="Australia"] = df.sigma$both[df.sigma$Country=="Australia"]
df.sigma$final[df.sigma$Country=="Canada"] = df.sigma$btc[df.sigma$Country=="Canada"]
df.sigma$final[df.sigma$Country=="Japan"] = df.sigma$both[df.sigma$Country=="Japan"]
df.sigma$final[df.sigma$Country=="Netherlands"] = df.sigma$both[df.sigma$Country=="Netherlands"]
df.sigma$final[df.sigma$Country=="Italy"] = df.sigma$hwc[df.sigma$Country=="Italy"]

# Add production function parameters
param_base = data %>% 
  select(Country, Year) %>% 
  mutate("alpha" = (0.99)^40, "gamma" = .5) %>%
  # Phi = 1 - theta in 1970
  merge(df1 %>% subset(Year == FirstYear) %>% mutate(phi = 1-theta) %>% select(Country, phi)) %>% 
  # Sigma are those obtained from regressions above
  merge(select(df.sigma, Country, sigma = final), by = "Country") %>% 
  # No Biased technical change
  mutate(a = NA)
```
...
param_base = param_base %>%
  merge(., df1, by = c("Country", "Year")) %>% 
  mutate(X = exp(-(sigma + ((1-gamma)/gamma+sigma)*(1-phi)/phi*k^((1-sigma)/sigma))^(-1)),
         theta = (1+phi/(1-phi)*k^((sigma-1)/sigma))^(-1),
         THETA = theta/(1-theta),
         eta = 1/(1-u)*(1/X/theta-1)^(-1),
         omega = p/n/(1+alpha*p1)*eta,
         beta = ((1+eta)*tau-u*eta)/(1-tau),# model2
         ) %>% 
  group_by(Country) %>% 
  mutate(omega = nth(omega, 3), beta = nth(beta, 3)) %>%
  # mutate_at(vars(omega, beta), ~ mean(., na.rm = T)) %>%
  select(Country, Year, FirstYear, alpha, gamma, phi, sigma, a, omega, beta) %>% 
  ungroup()
# Preview
subset(param_base, Year == FirstYear)
```
...
coef_summary = param_base %>% 
  ...
  mutate(
    variable = c("$\\phi$", "$\\gamma$", "$\\alpha$", "$\\sigma$", 
                 "$\\omega$", "$\\beta$", "$A$"),
    var_desc = c("Capital share in 1950", "Relative bargaining power of the union",
...
for(country in CountrySetAll[-7]){
    result = data %>%
...
graph_dev1080 = list.sim$benchmark %>% 
  ...
  geom_line() +
  facet_wrap(. ~ variable, scales = "free", nrow = 2, labeller = label_parsed) +
  scale_x_continuous(breaks = seq(2010, 2100, 10), labels = c("2010","", "2030", "", "2050", "", "2070", "", "2090", "")) +
...
```{r tab-demo70}
# Print demographic variables in 1950 in the table
tab_demo70 = list.sim$benchmark %>% 
  ...
  mutate(
    "var" = c("$n_{1970}$", "$p_{1970}$", "$p_{1990}$", "$\\frac{p_{1970}}{n_{1970}}$",
              "$\\eta_{1970}$"),
...
graph_deie_decomp +
  geom_line(aes(y = position_DEIE), color = "black") +
    scale_fill_grey(labels = deie_labels2, start = 0, end = .5)
...
```{r graph-decomp-sum2}
# Summary
graph_decomp_summary2 = decomp_mean.period %>% setDT %>% 
  melt(id.vars = c("Country_break", "Period")) %>% 
  mutate(value = value*100) %>% 
  subset(Country_break %in% c("1970 - France", "1970 - United States")) %>% 
  mutate(Country = ifelse(Country_break == "1970 - France", "France", "United States"),
...
# Paper version
graph_decomp_summary2 +
  geom_bar(stat = "identity", position = position_dodge(), alpha = .7) +
  scale_fill_grey(start = 0, end = .5)
  ggsave(file.path(loc_graphic, "quant-decomp-sum.png"), width = 7, height = 7/2)

# Color version
graph_decomp_summary2 +
    geom_bar(stat = "identity", position = position_dodge(), alpha = .7) +
    scale_fill_manual(values = brewer.pal(8, "Set1")[c(5,8,4)])
  ggsave(file.path(loc_graphic, "quant-decomp-sum-color.png"), 
         width = 7, height = 7/2)
```
...
redis = list.sim$benchmark %>%
  ...
  mutate(
    ...
    c1_emp = inc_emp/(1+alpha*p1),
    c1_unemp = inc_unemp/(1+alpha*p1),
    # Consumption 2nd period
    c2_emp = alpha*p1/(1+alpha*p1)*(1-lead(tau,1))*inc_emp*r/p,
    c2_unemp = alpha*p1/(1+alpha*p1)*(1-lead(tau,1))*inc_unemp*r/p,
    # Life-time utility
    utility_emp = log(c1_emp) + alpha*p1*(log(c2_emp) + beta*log(lead(h,1))),
    utility_unemp = log(c1_unemp) + alpha*p1*(log(c2_unemp) + beta*log(lead(h,1))),
    # Expected
    utility_exp = (1-u)*utility_emp + u*utility_unemp
    ) %>% 
  # Compute GDP split
  mutate(
    ...
    theta = wL/Y, dep = p/n,
    THETA = theta/(1-theta),
    inc_ratio = inc_y/inc_o,
    delta1 = inc_ratio-THETA,
    delta2 = 1/((1-theta)*(1/u/eta+(1-u)/u)),
    delta2_dep = 1/((1-theta)*(1/u/eta+(1-u)/u))*p/n,
    ) %>% 
  ungroup()
```
...
graph_utility = redis %>% 
  ...
  select(Country, Year, utility_exp, p1) %>% subset(Year %in% c(1950:2040)) %>% 
  # In % deviation from 1970
  group_by(Country) %>% 
  mutate(utility_exp = utility_exp %>% {(./.[Year == 1950] -1)*100}) %>% 
  # Melt (from wide to long)
  setDT %>% melt(id.vars = c("Country", "Year")) %>% 
  # Rename variables
...
```{r winners-ratio-pc, fig.cap = 'Per-capita redistribution from old to young'}
## Core
graph_ratio_pc = redis %>% 
  select(Country, Year, delta2_dep, delta2, dep) %>% 
  group_by(Country) %>% 
  # In % deviation from 1970
  mutate_at(vars("delta2_dep", "delta2", "dep"), ~ {(./.[Year == 1970] -1)*100}) %>% 
  # Select variables
  select(Country, Year, delta2_dep) %>% 
  # Melt (from wide to long)
  # setDT %>% melt(id.vars = c("Country", "Year")) %>%
  subset(Year >= 1970 & Year <= 2050) %>%
  
  ggplot(aes(x = Year, y = delta2_dep, color = Country)) +
  geom_rect(data = boomer_period2, inherit.aes = FALSE,
            aes(xmin = start, xmax = end, ymin = -Inf, ymax = Inf, fill = period),
            color = "transparent", alpha = .2) +
  geom_line() +
  ...
  scale_x_continuous(breaks = breaks_10_years) +
  theme_bw() +
  labs(x = element_blank(), y = element_blank(), title = "Per-capita redistribution from old to young (1970 = 0)", linetype = element_blank(), color = "Country", fill = "Boomers' period",
       fill = element_blank()) +
  theme(legend.position = "right")
...
for(spe in c(1:3)){
  ...
  data = data %>% 
    ...
    mutate(
      ...
      p1.new = ifelse(Year > break_year & Period < 4, dplyr::lead(p.new,1),
                      ifelse(Period == 4, p.new + p1_to_p_dist*frac, p1)),
...
list.graph$retage_p = list.sim$retage[,-c({ncol(list.sim$retage)-2}:ncol(list.sim$retage))] %>%
  ...
  scale_x_continuous(breaks = seq(2020, 2100, 10), labels = c("2020", "", "2040", "", "2060", "", "2080", "", "2100")) +
...
This section provides the details of the uniqueness of the equilibrium in the paper (appendix). I start by computing g and h functions for feasible values of parameters from the calibration.

```{r uniq-compute, warning=F}
# Define the g function
g = function(k, sigma, phi, Ny, eta, K){
  G = log((1+phi/(1-phi)*k^((sigma-1)/sigma))/(1+k/eta*Ny/K))
  return(G)
}
# Define h function
...
# Create dataframe with k
df.uniq = expand.grid(
  k = seq(0, 10, length.out = 10000),
  sigma = c(0.25, 0.8, 1, 1.1, 1.2, 1.3, 1.5, 1.7), 
  phi = c(.1, .3, .5, .7), gamma = c(.1, .3, .5, .7, .9), 
  Ny = c(1, 2, 3, 4, 5), K = c(1, 2, 3, 4, 5), eta = c(1, 2, 3, 4, 5)
  ) %>% 
  mutate(g = g(k, sigma, phi, Ny, eta, K), 
         h = h(k, sigma, phi, gamma))
```

Figure \@ref(fig:graph-uniq-gshape) shows the shapes of the g function. This figure corresponds to Figure 11 in the paper (appendix).

```{r graph-uniq-gshape, warning=F}
# Figure
df.uniq %>% 
  subset(sigma %in% c(.8, 1.2)) %>% 
  ...
  theme_bw()
  ggsave(file.path(loc_graphic, "uniq-gshape.png"), width = 7, height = 7/2)
```

Figure \@ref(fig:graph-uniq-hshape) shows the shapes of the h function.

```{r graph-uniq-hshape, warning=F}
# Figure
df.uniq %>% 
  subset(sigma %in% c(0.25, 0.8, 1.2)) %>% 
...
Figure \@ref(fig:graph-uniq-sigma1) shows the numerical simulation of $g(\tilde{k}_t)$ and $g(\tilde{k}_t)$, when both input factors are gross substitute, i.e. $\sigma > 1$ or $\rho \in (0,1)$. This figure corresponds to Figure 12 in the paper (appendix).

```{r graph-uniq-sigma1}
## Figure
graph_uniq = df.uniq %>% 
  subset(sigma %in% c(1.1, 1.2, 1.3, 1.5, 1.7)) %>% 
  subset(k <= 6) %>% 
  mutate(sigma = factor(sigma)) %>%
  setDT %>% 
  melt(id.vars= c("k", "sigma", "phi", "gamma", "Ny", "K", "eta")) %>% 
  
  ggplot(aes(x = k, y = value, color = sigma, linetype = variable)) +
  geom_line() +
  # Axis
  scale_x_continuous(breaks = 0) +
  scale_y_continuous(breaks = 0) +
  # Graphic parameters
  theme_bw() + theme(legend.position = "right") + 
  labs(x = element_blank(), y = element_blank(), linetype = "Function")


# Paper version
graph_uniq +
  scale_color_grey(name = bquote(sigma), start = 0, end  = .5)
  ggsave(file.path(loc_graphic, "uniq-sigmaplus1.png"), width = 7, height = 7/1.5)

# Color version
graph_uniq +
  scale_color_manual(name = bquote(sigma), values = brewer.pal(8, "Set1"))
  ggsave(file.path(loc_graphic, "uniq-sigmaplus1-color.png"), width = 7, height = 7/1.5)

```

```{r}
# Checking whether g and h intersect on the increasing part of g
df.uniq2 = df.uniq %>% 
  group_by(sigma) %>% 
  mutate(increase = g > lag(g)) %>% 
  mutate(violation = g < h) %>% 
  ungroup
  
# 0 TRUE & TRUE => Does not intersect
table(df.uniq2$increase, df.uniq2$violation)
```
...
screenreg(list.reg$sigma$Australia, stars = c(0.01, 0.05, 0.1), custom.coef.map = coef.map, digits = 3)
...
# Table from regressions with sigma
...
# Select France
pwt2.fr = subset(pwt2, Country == "Italy")
...
list.graph$regime_regline = pwt2.fr %>% 
  ...
  mutate(before_break = factor(ifelse(Year <= 1985, "1950-1985", "1986-2017")),
...
m0 = lm(THETA1_log ~ k.avh_log, data = pwt2.fr)
## Estimate the optimal break point
mbp = breakpoints(THETA1_log ~ k.avh_log, data = pwt2.fr, breaks = 1)
# Optimal break
...
m1 = lm(THETA1_log_detrend ~ k.avh_log_detrend*I(Year<=break_year), data = pwt2.fr)
...
tabular(reg2, coef_map = coef.map2,
        ...
        digits = 3,
        group.models = list("1950-1985" = 1:2, "1986-2017" = 3:4),
