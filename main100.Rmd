...
CountrySet1980 = c("Denmark", "Japan", "Netherlands", "Italy")
...
```{r quant-init}
# Simulation periods
sim = seq(1950, 2100, 10)
# Estimation sample
est_sample = c(1950:2010)
# Graphic parameters
...
pwt0 = pwt %>%
  ...
  subset(Country %in% CountrySet & Year >= 1950)
# OECD data
oecd0 = oecd %>% 
...
data_base = df1 %>% 
  ...
  mutate(Sequence = ((sim - 1950)/10) %% 4 + 1, Period = (sim - init_seq) / 40 + 1) %>%
  # Remove values for Ny, No and K after the 1st periods
  mutate_at(vars("Ny", "No", "K"), ~ ifelse(Year >= 1990, NA, .)) %>% 
  ...
  merge(., summarise(., p1.growth = mean(p1/lag(p1)-1, na.rm = T)), by = "Country") %>% 
  group_by(Country) %>% 
  mutate(p1 = ifelse(is.na(p1) & Year > 2060, 
                     p1[Year == 2060]*(1+p1.growth)^Sequence, p1)) %>% 
  ungroup
# Re-order variables
data_base = data_base %>% 
...
param_base = data.frame(
  "Country" = rep(CountrySet, each = length(sim)),
...
df1.sigma0 = df1 %>% 
  ...
  subset(Year %in% est_sample) %>% 
  ungroup()

#
df.sigma = data.frame(Country = CountrySet)
for(country in CountrySet){
  
  df.sigma$all[df.sigma$Country == country] = lm(THETA_log ~ k_log, data = df1.sigma0, 
...
param_base = param_base %>%
  # Phi = 1 - theta in 1970
  merge(df1 %>% subset(Year == 1950) %>% mutate(phi = 1-theta) %>% select(Country, phi)) %>% 
  # Sigma are those obtained from regressions above
  merge(rename(df.sigma, sigma = all), by = "Country") %>% 
  # No Biased technical change
  mutate(a = NA)
```
...
param = param_base
# Preview
param_base[seq(1, nrow(param_base), by = 16),]
```
...
boomer_period = data.frame(
  x1 = c(1970, 2010), x2 = c(2010, 2050), y1 = c(-100, -100), y2 = c(100, 100),
  period = factor(c("Young boomers", "Old boomers"), levels = c("Young boomers", "Old boomers"))
)
```
...
graph_ratio_pc = redis %>% 
  ...
  group_by(Country) %>% 
  # In % deviation from 1970
  mutate_at(vars("delta2_dep", "delta2", "dep"), ~ {(./.[Year == 1950] -1)*100}) %>% 
  # Select variables
  ...
  geom_rect(data = boomer_period, inherit.aes = FALSE,
            aes(xmin = x1, xmax = x2, ymin = y1, ymax = y2, fill = period),
            color = "transparent", alpha = .3) +
  scale_x_continuous(breaks = breaks_10_years, labels = labs_20_years) +
  theme_bw() +
  labs(x = element_blank(), y = element_blank(), title = "Per-capita redistribution from old to young (1950 = 0)", linetype = element_blank(), color = element_blank(),
       fill = element_blank()) +
  theme(legend.position = "bottom")
...
graph_ratio_pc + scale_color_grey(start = 0, end = .5) +
  scale_fill_grey(start = 0, end = .5)
  ggsave(file.path(loc_graphic, "discuss-ratiopc.png"), width = 7, height = 7/1.5)

# Paper version
graph_ratio_pc + scale_color_brewer(palette = "Set1") +
  scale_fill_manual(values = brewer.pal(8, "Set1")[c(8,5)])
  ggsave(file.path(loc_graphic, "discuss-ratiopc-color.png"), width = 7, height = 7/1.5)
