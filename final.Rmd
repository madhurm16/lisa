...
df %>% 
  subset(Year %in% c(1970:2010)) %>% 
  select(Country, Year, ls = labsh1, US100, UC1000, US100.lin_inter, UC1000.lin_inter, dep) %>% 
  subset(Country %in% c("France", "United States")) %>%
  mutate(urep = UC1000*100) %>% 
...
df %>% 
  subset(Year %in% c(1970:2010)) %>% 
  select(Country, Year, tax_rev_PC_GDP, dep) %>% 
  subset(Country %in% c("Australia", "Belgium", "Canada", "Denmark", "France", "Germany", 
                        "Japan", "United States", "Netherlands", "United Kingdom")) %>%
  mutate(tau = tax_rev_PC_GDP) %>% 
  
  ggplot(aes(x = dep, y = tau, color = Country)) +
  geom_point() +
  geom_smooth(method = "lm", formula = y ~ x) +
  labs(x = "Old-age dependency ratio", y = element_blank(), title = "Government share of GDP (in %)") +
  theme_bw()
```
...
graph_dev7010 = list.sim$baseline %>% 
  ...
  mutate_at(vars(eta, outside, w, L, u, K, Y), ~ (./.[Year == 1970]-1)*100) %>% 
  subset(Year %in% c(1970: 2010)) %>% 
  # Variables and names
  select(Country, Year,
